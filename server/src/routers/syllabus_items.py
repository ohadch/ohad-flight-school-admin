...
from src.schemas.syllabus_item import SyllabusItemSchema, SyllabusItemCreateSchema, SyllabusItemUpdateSchema
...
async def read_syllabus_item(syllabus_id: int, item_id: int, db: Session = Depends(get_db)):
    syllabus = db.query(Syllabus).get(syllabus_id)
    return syllabus.items.get(item_id)


@router.post("/", response_model=SyllabusItemSchema)
async def create_syllabus_item(syllabus_id: int, syllabus_item_schema: SyllabusItemCreateSchema, db: Session = Depends(get_db)):
...
async def update_syllabus_item(syllabus_id: int, item_id: int, syllabus_item_schema: SyllabusItemUpdateSchema, db: Session = Depends(get_db)):
    syllabus = db.query(Syllabus).get(syllabus_id)
...
async def delete_syllabus_item(syllabus_id: int, item_id: int, db: Session = Depends(get_db)):
